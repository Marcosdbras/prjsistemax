...
function ic_shortcode_mode_settings() {
	...
	if ( is_ic_shortcode_integration() ) {
		...
		if ( is_ic_breadcrumbs_enabled() ) {
			implecode_settings_checkbox( __( 'Move breadcrumbs to the top', 'ecommerce-product-catalog' ), 'archive_multiple_settings[shortcode_mode][move_breadcrumbs]', $settings[ 'move_breadcrumbs' ], 1, __( 'Breadcrumbs will be displayed before page title. It may require some additional styling when checked.', 'ecommerce-product-catalog' ) );
		}
