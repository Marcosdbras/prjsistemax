...
function shipping_menu() {
	?>
	<a id="shipping-settings" class="nav-tab" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=shipping-settings&submenu=shipping' ) ?>"><?php _e( 'Shipping', 'ecommerce-product-catalog' ); ?></a>
	<?php
}
		...
		<div class="settings-submenu">
			...
			<div class="setting-content submenu">
				...
				<form method="post" action="options.php">
					...
					if ( $shipping_count > 0 ) {
						?>
						<div class="al-box info"><p><?php _e( "If you fill out the fields below, system will automatically pre-fill the fields when adding new item so you doesn't have to fill them every time.</p><p>When every item in your catalogue has different shipping options you can leave all or just a part of these fields empty.", 'ecommerce-product-catalog' ); ?></p><p><?php _e( 'You can change these default values for every new item in the catalog.', 'ecommerce-product-catalog' ); ?></p></div>
