...
<script type="application/ld+json">
	...
	"image": "<?php echo get_product_image_url( $product_id ) ?>",
	"description": "<?php echo get_product_short_description( $product_id ) ?>",
	<?php do_action( 'ic_structured_data', $product_id ) ?>
	"offers": {
	"@type": "Offer",
	"url": "<?php echo get_permalink() ?>",
	"priceCurrency": "<?php echo get_product_currency_code() ?>",
	"price": "<?php echo product_price( $product_id ) ?>",
	"priceValidUntil": "<?php echo date( 'Y-m-d', strtotime( date( "Y-m-d", time() ) . " + 365 day" ) ); ?>",
	"itemCondition": "https://schema.org/NewCondition",
	"availability": "https://schema.org/InStock",
	<?php do_action( 'ic_structured_data_offers', $product_id ) ?>
	"seller": {
	"@type": "Organization",
	"name": "<?php echo get_bloginfo( 'name' ) ?>"
	<?php do_action( 'ic_structured_data_seller', $product_id ) ?>
	}
	}
	}
</script>
