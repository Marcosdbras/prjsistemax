...
type
  Tfrmadditem = class(TForm)
    ...
    img_desistir_lancar_item: TImage;
    ...
    procedure img_desistir_lancar_itemClick(Sender: TObject);
...
for x := 1 to 10 do
  AddProdutolistview(x,idgrupo.ToString +'Produto yyyyy yyyy yyyyyy yyyyy yyyyy yyyyy yyyyy yyyyyy yyyy'+x.ToString,x);
...
procedure Tfrmadditem.lsvprodutoItemClick(const Sender: TObject;
  const AItem: TListViewItem);
begin
  //Exibir confirmação e quantidade
  edtqtde.Text := '01';
...
procedure Tfrmadditem.img_desistir_lancar_itemClick(Sender: TObject);
begin
  layout_confirmaitem.Visible := false;
end;
...
begin
   try
     ...
     if edtqtde.Text.ToInteger < 1 then
      edtqtde.Text := '01';
